...
internal class SqlStateStorage : IStateStorage
{
    private static bool _initialized = false;
    private readonly IServiceScope _provider;

    public SqlStateStorage(IServiceProvider provider) => _provider = provider.CreateScope();
    ...
    private async Task<SqlStateContext> GetSqlStateContext()
    {
        var context = _provider.ServiceProvider.GetRequiredService<SqlStateContext>();
        if(!_initialized)
        {
            _initialized = true;
