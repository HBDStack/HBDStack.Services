...
internal class MySqlStateStorage : IStateStorage
{
    private static bool _initialized = false;
    private readonly IServiceScope _provider;

    public MySqlStateStorage(IServiceProvider provider) => _provider = provider.CreateScope();
    ...
    private async Task<MySqlStateContext> GetSqlStateContext()
    {
        var context = _provider.ServiceProvider.GetRequiredService<MySqlStateContext>();
        if(!_initialized)
        {
            _initialized = true;
